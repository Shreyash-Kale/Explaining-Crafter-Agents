...
Note: launch the UI with `python -m vis.main` from the project root. Do not run `python vis/main.py` directly.
...
- Deterministic template-based explanation toolbox for step-level natural language narration.
...
- `Show Charts`: right pane shows plots, bottom pane shows decision attribution plot.
- `Show Achievements`: right pane shows achievements, bottom pane shows the Explanation Toolbox.

## Repository Map

- Training entry point: `dreamer/train.py`
- Policy and replay: `dreamer/policy.py`
- DreamerV2 core models: `dreamer/core.py`
...
The Explanation Toolbox is synced to timeline/video step updates and narrates each step using template-based text.
...
Recent visualization additions:

- New module: `vis/explainer.py` with deterministic `generate_explanation(...)`.
- Bottom-pane mode switch in `vis/main.py` now shows explanation text while in Achievements mode.
...
This `README.md` is intentionally concise. The project guide is the source of truth for architecture and analysis details.
