...
> **Document status:** This file started as a *planning* document. The template-based
> explainer it proposed is now **implemented** — see **"As-Built: What Actually Shipped"**
> below for the real API and wiring, which differs from the original proposals.
> The paper analysis (sections that follow immediately) remains valid as theoretical
> grounding. The "Integration Plan 1/2" sections near the end are preserved as the
> *original proposals* and are explicitly marked as historical — read the As-Built
> section for the current code.
...
This section documents the explanation system **as implemented in the current code**.
Where it differs from the original proposals (below), this section wins.

## Implemented: `vis/explainer.py`

A deterministic, dependency-free template engine. Public API:

```python
generate_explanation(step_row, prev_row=None, algorithm=None) -> str
infer_algorithm(step_row) -> "dreamer" | "ppo" | "unknown"
```

Key differences from the original proposals:

- The real `generate_explanation` takes **dict-like rows** (`step_row`, `prev_row`),
  not the long flat argument list (`step, action, action_prob, value_curr, ...`) the
  plans proposed. It pulls fields out of the row itself and tolerates missing keys via
  `_safe_float`.
- Algorithm auto-detection lives **in `explainer.py` as `infer_algorithm()`**, not as
  `detect_algorithm()` in `data_manager.py`. It inspects which columns exist:
  `exploration_bonus` + `world_model_score` → dreamer; `entropy` + `advantage` → ppo.
- Output is multi-line (`"\n".join(...)`), covering: action + confidence, value
  trend, reward context, algorithm-specific signal (world-model text for Dreamer,
  entropy + advantage text for PPO), vital-stat drop warnings, and achievement unlocks.

Tunable thresholds (module constants, current values):

```python
CONFIDENCE_HIGH = 0.75      CONFIDENCE_MEDIUM = 0.50
VALUE_DELTA_THRESHOLD = 0.05
WORLD_MODEL_HIGH = 0.75     WORLD_MODEL_MEDIUM = 0.40
ENTROPY_HIGH = 0.60         ADVANTAGE_POSITIVE = 0.10   ADVANTAGE_NEGATIVE = -0.10
```

Standalone preview (no GUI needed):

```bash
python vis/explainer.py data.csv --rows 20            # auto-detect algorithm
python vis/explainer.py data.csv --rows 20 --algorithm ppo
```

## Implemented: wiring

- **`vis/widgets.py`** — `ExplanationPanel` renders the text; the decision-attribution
  plot draws `value_estimate` (deep violet), `action_probability` (dusty rose),
  `exploration_bonus` (amber-brown), and `world_model_score` (slate blue), with PPO
  `entropy`/`advantage` swapped in when present.
- **`vis/main.py`** — calls `explanation_panel.update_step(step)` on every timeline /
  video frame change, and infers an `algorithm_hint` at load time from the normalized
  signals (`get_dreamer_explore_norm` / `get_ppo_entropy_norm`).
- **`vis/data_manager.py`** — exposes normalized signal accessors
  (`get_dreamer_explore_norm`, `get_dreamer_wm_score_norm`, `get_ppo_entropy_norm`,
  `get_ppo_advantage_norm`) rather than a single `detect_algorithm()` helper.

## Still proposals (not yet built)

- **LLM enrichment** of the template text (the `enrich_with_llm` / ollama idea).
- **Critical-state flagging** and the **prediction task** study integration.
- **`SemanticEventDetector` is NOT wired into the load path.** It is fully implemented
  in `vis/SemanticEventDetector.py` but nothing imports or calls it yet, so it does not
  currently feed the `achievement_unlocked` field. Wiring it in remains a clean
  follow-up (the explainer already reads `achievement` / `achievement_unlocked` if present).

---

# (Historical proposal) Integration Plan 1 -

> ⚠️ **Historical.** The plan below predates the implementation and assumed the repo
> was inaccessible. It is kept for context only. For the real code, see
> **"As-Built: What Actually Shipped"** above — the actual `generate_explanation`
> signature and the location of algorithm detection both differ from this plan.
...
# (Historical proposal) Integration Plan 2 -

> ⚠️ **Historical.** A second proposed wiring, also superseded by the shipped code.
> Notably, the real engine does **not** use the flat-argument `generate_explanation(step,
> action, action_prob, ...)` shown here — it takes `step_row`/`prev_row` dicts — and
> `detect_algorithm()` was implemented as `infer_algorithm()` inside `explainer.py`,
> not in `data_manager.py`. See **"As-Built"** above.
